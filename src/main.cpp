...
int main(int argc, char* argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "l:vVh", long_options, &option_index)) != -1) {
        switch (opt) {
            case 0: {
                ...
                if (name == "diameter-port")
                    config.diameterPort = std::stoi(optarg);
                else if (name == "diameter-peer-host")
                    config.diameterPeerHost = optarg;
                else if (name == "diameter-peer-port")
                    config.diameterPeerPort = std::stoi(optarg);
                else if (name == "origin-host")
                    config.originHost = optarg;
                else if (name == "origin-realm")
                    config.originRealm = optarg;
                else if (name == "product-name")
                    config.productName = optarg;
                else if (name == "dictionary")
                    config.dictionaryPaths.push_back(optarg);
                else if (name == "watchdog-interval")
                ...
                else if (name == "http2-server-port")
                    config.http2ServerPort = std::stoi(optarg);
                else if (name == "workers")
                    config.workers = std::stoi(optarg);
                // else if (name == "admin-port") config.adminPort = std::stoi(optarg); // NOT YET IMPLEMENTED
                else if (name == "prometheus-port")
    ...
    else if (logLevel == "Informational")
        ert::tracing::Logger::setLevel(ert::tracing::Logger::Informational);
    else if (logLevel == "Notice")
        ert::tracing::Logger::setLevel(ert::tracing::Logger::Notice);
    else if (logLevel == "Warning")
        ert::tracing::Logger::setLevel(ert::tracing::Logger::Warning);
    else if (logLevel == "Error")
        ert::tracing::Logger::setLevel(ert::tracing::Logger::Error);
    ...
    std::cout << "Dictionary: ";
    if (config.dictionaryPaths.empty()) {
        std::cout << "<not provided>" << std::endl;
    } else {
        std::cout << std::endl;
        for (const auto& p : config.dictionaryPaths) std::cout << "  " << p << std::endl;
    }
    ...
    // std::cout << "Admin port: " << config.adminPort << std::endl; // NOT YET IMPLEMENTED
    std::cout << "Prometheus port: " << (config.metricsEnabled ? std::to_string(config.prometheusPort) : "disabled")
              << std::endl;
    std::cout << std::endl;
    ...
    // Keep io_context alive (HTTP/2 server uses its own threads via nghttp2)
